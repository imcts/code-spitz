  ...
  const parse = (html, node) => {
    ...
    switch (str[0]) {
      case '<':
        index = str.indexOf('>');
        if (str[1] === '/') {
          return parse(str.slice(index + 1), node?.parentElement ?? node);
        } else {
          const dom = tagParser.route(str.slice(0, index))
          if (node) {
            node.appendChild(dom)
          }
          return parse(str.slice(index + 1), str[index - 1] === '/' ? (node ?? dom) : dom);
        }
      default:
        index = str.indexOf('<');
        const text = document.createTextNode(str.slice(0, index).trim());
        node.appendChild(text);
        return parse(str.slice(index), node)
    }
  }
  return html => {
    if (!html) {
