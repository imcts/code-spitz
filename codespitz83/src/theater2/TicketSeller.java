...
public class TicketSeller {
  ...
  Reservation reserve(Customer customer, Theater theater, MovieTheater movieTheater, Movie movie, Screening screening, int count) {
    Reservation reservation = Reservation.NONE;
    Money price = movie.calculateFee(screening, count);
    if (customer.hasAmount(price)) {
      reservation = ticketOffice.reserve(theater, movieTheater, movie, screening, count);
